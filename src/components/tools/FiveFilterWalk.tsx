...
import { C, fs, PrimaryButton, SERIF, Tap } from '../../ui'
...
export default function FiveFilterWalk({
  ...
  return (
    <ToolPanel title={`🧭 ${WALK.title}`}>
      <div style={{ fontSize: fs(11), color: C.faint, marginTop: 4 }}>
        ...
        {([['AGENT — must register', true], ['NOT AN AGENT', false]] as const).map(([label, value]) => (
          <Tap
            ...
            style={{
              ...
              fontSize: fs(12),
              fontWeight: 700,
              background: answered && value === scenario.agent ? 'var(--k0e1f16)' : C.raisedAlt,
              color: answered ? (value === scenario.agent ? C.green : C.ghost) : C.textSerif,
              border: `1px solid ${answered && value === scenario.agent ? 'var(--k2a5c40)' : C.borderRaised}`,
            }}
          >
            {label}
          </Tap>
      ...
      {answered && (
        <>
          ...
          <PrimaryButton
            ...
            style={{ marginTop: 8, padding: 10, fontSize: fs(13) }}
          >
