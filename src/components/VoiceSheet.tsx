...
export default function VoiceSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Sheet onClose={onClose} maxHeight="60vh">
      <div style={{ fontFamily: MONO_DISPLAY, fontSize: fs(16), fontWeight: 700 }}>Narration voice</div>
      ...
      <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
        {voices.length === 0 && (
          ...
          >
            <span style={{ fontSize: fs(13), fontWeight: 500 }}>System default</span>
            <span style={{ fontSize: fs(11), color: C.faint }}>voices load after first tap on iPhone</span>
          </Tap>
        )}
        {voices.map((v) => {
          ...
          return (
            <Tap
              ...
              style={{
                ...
                borderRadius: 10,
                padding: '11px 13px',
                background: current ? 'var(--k1e2436)' : C.panel,
                border: `1px solid ${current ? 'var(--k3a4a76)' : C.border}`,
              }}
            >
              <span style={{ fontSize: fs(13), fontWeight: 500 }}>
                {v.name.replace(/Microsoft |Google |Apple /, '')}
              </span>
              <span style={{ fontSize: fs(11), color: C.faint }}>
                {v.lang}
