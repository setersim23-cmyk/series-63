...
import { BackLink, C, MONO_DISPLAY, PrimaryButton, SERIF, Tap, Title } from '../ui'
      ...
      {finished ? (
        <>
          ...
          >
            ...
            <div style={{ fontFamily: SERIF, fontSize: 13, lineHeight: 1.65, color: '#c9d8ce', marginTop: 6 }}>
              {theCase.close}
            </div>
          </div>
          <PrimaryButton onClick={backToCases} style={{ marginTop: 12, padding: 12, fontSize: 13 }}>
            Back to the case files
          </PrimaryButton>
        </>
      ) : (
        <>
          <div style={{ fontSize: 14, fontWeight: 600, marginTop: 14 }}>{step.p}</div>
              ...
              return (
                <Tap
                  ...
                  style={{
                    ...
                    fontSize: 13,
                    lineHeight: 1.5,
                    background: answered ? (option.ok ? '#0e1f16' : picked ? '#210f14' : C.panel) : C.panel,
                    border: `1px solid ${
                      answered ? (option.ok ? '#2a5c40' : picked ? '#5c2a38' : C.border) : C.border
                    }`,
                    color: answered ? (option.ok ? '#b9edcd' : picked ? '#ffc4d0' : C.ghost) : C.textSerif,
                  }}
                >
                  {option.t}
                </Tap>
          ...
          {answered && (
            <>
              ...
              >
                <div
                  style={{ fontSize: 11, fontWeight: 700, color: step.o[state.pick!].ok ? C.green : C.pink }}
                >
